...
public class Application {
    public static void main(String[] args) {
        ...
        제네릭은 클ㄹ스나 메소드에서 사용할 내부 데이터 타입을 컴파일 시에 지정하는 방법이다
        컴파일 시에 미리타입걸사를 시행하므로 클래스나 메소드 내부에서 사용되는 객체의 타입 안정성을 높일 수 있다
        (잘못된 타입일 경우 컴파일 에러를 발생시티기 때문이다
        따라서 반환 값에 대한 타입 변환 및 타입 검사에 들어가는 코드 생략 가능
        [ 제네릭 프로그래밍 ]
